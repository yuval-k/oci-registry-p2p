...
podman run -d -p 5000:5000 --rm --name systemdtest -v ${PROJ_DIR}/oci-registry-p2p:/usr/local/bin/oci-registry-p2p:ro \
...
trap "podman stop systemdtest" EXIT

podman exec systemdtest /bin/sh -c "systemctl daemon-reload && systemctl enable oci-registry-p2p && systemctl start oci-registry-p2p"


podman exec systemdtest /bin/sh -c "journalctl -u oci-registry-p2p"
