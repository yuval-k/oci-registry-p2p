...
ALTNAMES = DNS:\$FQDN , IP:104.196.167.27   # , DNS:bar.example.org , DNS:www.foo.example.org
...
openssl req -nodes -newkey rsa:2048 -keyout $TMPDIR/key.pem -out $TMPDIR/cert.csr -config $TMPDIR/openssl.cnf -reqexts req_ext
# cat $TMPDIR/cert.csr
# openssl req -in $TMPDIR/cert.csr -noout -text
echo
echo Generating cert
openssl x509 -in $TMPDIR/cert.csr -out $TMPDIR/cert.pem -req -signkey $TMPDIR/key.pem -days 3650 -extfile $TMPDIR/openssl.cnf -extensions req_ext


echo certs created:
cp $TMPDIR/key.pem key.pem
cp $TMPDIR/cert.pem cert.pem
